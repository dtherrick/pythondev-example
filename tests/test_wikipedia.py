...
from pythondev_example import wikipedia


def test_random_page_uses_given_language(mock_requests_get: Mock) -> None:
    """Test that confirms selection of the specified Wikipedia language edition."""
...
def test_random_page_returns_page(mock_requests_get: Mock) -> None:
    """Test that confirms we return an object of type Page."""
    page = wikipedia.random_page()
    assert isinstance(page, wikipedia.Page)


def test_random_page_handles_validation_errors(mock_requests_get: Mock) -> None:
    """Test to confirm that we raise a `ClickException` when validation fails."""
